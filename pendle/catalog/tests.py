...
class TestReservationDuration(TestCase):
    ...
    def setUp(self):
        self.weekdays_9_5 = Period.objects.get(pk=1)
        self.weekdays_1pm = Period.objects.get(pk=2)
        self.durations = ReservationDuration.objects.in_bulk(range(1, 11))
    ...
    def test_get_due_date_for_1_hour_end_duration_is_end_of_next_period_at_least_1_hour_from_now(self):
        ...
        now = datetime(2011, 1, 1, 12, 0, 0)
        due_date = duration.get_due_date(now, [self.weekdays_9_5])
    ...
    def test_ime_period_9am(self):
        duration = self.durations[6]
        now = datetime(2011, 1, 5, 9, 0, 0) # Wednesday
        due_date = duration.get_due_date(now, [self.weekdays_1pm])
        self.assertEqual(due_date, datetime(2011, 1, 6, 13, 0, 0))

    def test_ime_period_1pm(self):
        duration = self.durations[6]
        now = datetime(2011, 1, 5, 13, 0, 0) # Wednesday
        due_date = duration.get_due_date(now, [self.weekdays_1pm])
        self.assertEqual(due_date, datetime(2011, 1, 6, 13, 0, 0))

    def test_ime_period_5pm(self):
        duration = self.durations[6]
        now = datetime(2011, 1, 5, 17, 0, 0) # Wednesday
        due_date = duration.get_due_date(now, [self.weekdays_1pm])
        self.assertEqual(due_date, datetime(2011, 1, 6, 13, 0, 0))

    def test_ime_period_saturday(self):
        duration = self.durations[6]
        now = datetime(2011, 1, 1, 12, 0, 0) # Saturday
        due_date = duration.get_due_date(now, [self.weekdays_1pm])
        self.assertEqual(due_date, datetime(2011, 1, 3, 13, 0, 0))
...
class TestPeriod(TestCase):
    ...
    def setUp(self):
        self.period_weekdays_9_5 = Period.objects.get(pk=1)
        self.period_1pm = Period.objects.get(pk=2)
